...
# Load the GPT-2 tokenizer (should match your model's tokenizer if possible)
...
def run_evaluation(problems: List[Dict[str, str]], iterations: int, k: int, max_length: int):
    """Evaluate both methods on the given problems and plot the results."""
...
def load_math_problems(sample_size: int) -> List[Dict[str, str]]:
    """Load a sample of problems from the MATH dataset that have boxed answers."""
    dataset = load_dataset("hendrycks/math", "algebra", split="train")
    problems = []
    for item in dataset:
        if "\\boxed{" in item["solution"]:
        ...
        if len(problems) >= sample_size:
            break
    if len(problems) < sample_size:
        print(f"Warning: Only {len(problems)} problems with boxed answers found.")
    return problems

if __name__ == "__main__":
    # Configuration parameters
    ITERATIONS = 2  # Number of MCTS iterations
    K = 2          # Top-k tokens to explore
    MAX_LENGTH = 10  # Maximum sequence length
    SAMPLE_SIZE = 5   # Number of problems to evaluate
    
    # Load problems from the MATH dataset
    problems = load_math_problems(SAMPLE_SIZE)
    
    # Run the evaluation
    run_evaluation(problems, ITERATIONS, K, MAX_LENGTH)